...
struct internal_ldsym
{
  union
  {
    /* The symbol name if <= COFF_SYMNMLEN(bfd) characters.  */
    char _l_name[MAX_SYMNMLEN];
    struct
    {
      /* Zero if the symbol name is more than COFF_SYMNMLEN(bfd) characters.  */
      bfd_vma _l_zeroes;
      
      /* The offset in the string table if the symbol name is more
	 than COFF_SYMNMLEN(bfd) characters.  */
      bfd_vma _l_offset;
    } 
    _l_l;
  }
