...
#ifdef TC_SX
  /* the IEEE quad precision floating-point numbers in the following format
     in NEC SX systems:
     - bit 0: sign
     - bits 1-11: exponent
     - bits 12-63: upper part of mantissa
     - bits 64-75: undefined
     - bits 76-127: lower part of mantissa */
  else if (precision == Q_PRECISION)
    {
      /* write upper part of mantisa, the bits 16-63 (the bits 12-15 were 
         already written as a part of "word1") */
      int i;
      for (i=0; i<3; i++)
        *lp++ = next_bits (LITTLENUM_NUMBER_OF_BITS);
      /* the bits 64-75 are undefined. we set them to 0. We also assign
       * the mantissa bits to the lowest 4 bits of this word. */
      *lp++ = next_bits ((LITTLENUM_NUMBER_OF_BITS - 1) - exponent_bits);
    }
#endif
...
#ifdef TC_SX
          if (precision == Q_PRECISION && lp == (words + 4))
            {
              /* on the NEC SX, there is a gap between bits 64 and 75.
                 We must explicitly propagate the carry into the exponent. */
              *lp = carry & 0xf;
              carry >>= 4;
            }
          else
