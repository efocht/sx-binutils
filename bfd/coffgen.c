				...
				char *buf)
{
  ...
  if (sym->_n._n_n._n_zeroes != 0
    ...
    {
      memcpy (buf, sym->_n._n_name, bfd_coff_symnmlen(abfd));
      buf[bfd_coff_symnmlen(abfd)] = '\0';
      return buf;
    }
  else
    {
  ...
  if (native->u.syment.n_sclass == C_FILE
      ...
      if (bfd_coff_force_symnames_in_strings (abfd))
	{
	  ...
	  *string_size_p += 6;  /* strlen(".file") + 1 */
  	...
  	strncpy (native->u.syment._n._n_name, ".file", bfd_coff_symnmlen(abfd));
	  ...
	  if (internal_ptr->u.syment._n._n_n._n_zeroes != 0)
	    {
	      ...
	      /* Find the length of this string without walking into memory
	         that isn't ours.  */
	      for (i = 0; i < bfd_coff_symnmlen(abfd); ++i)
	  ...
	  for (aux = 0; aux < combined->u.syment.n_numaux; aux++)
		...
		case C_STAT:
		  if (combined->u.syment.n_type == T_NULL)
		    ...
		    {
		      fprintf(file, "AUX scnlen 0x%" BFD_VMA_FMT "x"
			      " nreloc %lu nlnno %lu",
		...
		case C_EXT:
		  if (ISFCN (combined->u.syment.n_type))
			...
			next = auxp->u.auxent.x_sym.x_fcnary.x_fcn.x_endndx.l;
		      ...
		      fprintf(file, "AUX tagndx %ld ttlsiz 0x%" BFD_VMA_FMT "x"
			      " lnnos %ld next %ld",
		  ...
		  /* Otherwise fall through.  */
		default:
		  fprintf (file, "AUX lnno %ld size 0x%lx tagndx %ld",
  ...
  if (p < pend)
    {
      ...
      /* FIXME: as we are truncating a 64-bit address to 32 bits
	 in order to have the thing build OK on 32-bit hosts,
	 if offset into the strings table is larger than 2^32
	 (this seems unlikey, though), this *won't* work.
	 actually, this seems not to be simple to fix without an
	 in-depth re-work of libbfd symbol name handling. 
      */
      *filename_ptr = (char *)(bfd_hostptr_t)p->u.syment._n._n_n._n_offset;
  ...
  if (section->lineno != NULL)
      ...
      for (; i < section->lineno_count; i++)
	{
	 ...
	 last line has executable code.  */
      /* NOTE: actually, we increased the "slop" to account for COFF/SX,
	 where 0x100 does not seem to suffice */
      if (i >= section->lineno_count
	  && last_value != 0
	  && offset - last_value > 0x200)
	{
