  ...
  while (esym < esymend)
    {
      ...
      char buf[MAX_SYMNMLEN + 1];
      const char *symname;

      bfd_coff_swap_sym_in (abfd, esym, & isym);

      if (sizeof (internal_s->s_name) > bfd_coff_symnmlen(abfd))
	{
...
.#define bfd_coff_symnmlen(abfd) (coff_backend_info (abfd)->_bfd_symnmlen)
...
#ifdef NEC_SX_COFF
  coff->f_flags2 = internal_f->f_flags2;
  coff->f_flags3 = internal_f->f_flags3;
  coff->f_flttype = internal_f->f_flttype;
  
  struct internal_aouthdr *internal_a =
	(struct internal_aouthdr *) aouthdr;
  if (internal_a != NULL) {
  	coff->aout_magic = internal_a->magic;
  	coff->ssize = internal_a->ssize;
  }
...
coff_set_arch_mach_hook (bfd *abfd, void * filehdr)
{
  ...
  switch (internal_f->f_magic)
    ...
    case LYNXCOFFMAGIC:
     ...
     case SX3MAGIC:
       arch = bfd_arch_sx;
       machine = bfd_mach_sx3;
       break;
     case SX4MAGIC:
       arch = bfd_arch_sx;
       machine = bfd_mach_sx4;
       break;
     case SX5MAGIC:
       arch = bfd_arch_sx;
       machine = bfd_mach_sx5;
       break;
     case SX8MAGIC:
       arch = bfd_arch_sx;
       machine = bfd_mach_sx8;
       break;
     case SX9MAGIC:
       arch = bfd_arch_sx;
       machine = bfd_mach_sx9;
       break;
#endif
  ...
  switch (bfd_get_arch (abfd))
	...
	* flagsp |= F_INTERWORK;
    ...
    case bfd_arch_sx:
      switch (bfd_get_mach (abfd))
        {
        case bfd_mach_sx3:  *magicp = SX3MAGIC;  return TRUE;
        ...
        case bfd_mach_sx9:  *magicp = SX9MAGIC;  return TRUE;
        default: break;
        }
      return TRUE;
#endif /* NEC_SX_COFF */
...
#ifdef NEC_SX_COFF
  internal_f.f_flags2 = coff_data(abfd)->f_flags2;
  internal_f.f_flags3 = coff_data(abfd)->f_flags3;
  internal_f.f_flttype = coff_data(abfd)->f_flttype;
#endif /* NEC_SX_COFF */
...
#undef __A_MAGIC_SET__
  ...
  internal_a.magic = coff_data(abfd)->aout_magic;
  internal_a.ssize = coff_data(abfd)->ssize;
#endif /* NEC_SX_COFF */
  ...
  for (counter = 0; counter < asect->lineno_count; counter++)
      ...
      if (cache_ptr->line_number == 0)
	{
	  ...
	  /* FIXME: We should not be casting between ints and
	     pointers like this.  */
	  sym = ((coff_symbol_type *)(bfd_hostptr_t)
...
coff_slurp_symbol_table (bfd * abfd)
{
  ...
  if (table_ptr == NULL)
      ...
      while (this_index < last_native_index)
	{
	  ...
	  /* FIXME: this will not work for COFF/SX, if offset field (which is
	     64 bit only on that arch) contains a value larger than 2^32.
	     since these are offsets into sections, this seems highly unlikely.
	  */
	  dst->symbol.name =
          (char *) (bfd_hostptr_t) (src->u.syment._n._n_n._n_offset);
...
#if !defined (TIC80COFF) && !defined (TICOFF) && !defined(NEC_SX_COFF)
#ifdef C_AUTOARG
...
#ifdef NEC_SX_COFF
	      /* NEC SX specific storage classes */
	    case C_GLCOMM:
	    case C_GSLCOMM:
	      dst->symbol.flags = BSF_GLOBAL;
	      dst->symbol.value = (src->u.syment.n_value);
...
static const bfd_coff_backend_data ticoff0_swap_table =
{
  ...
  coff_SWAP_filehdr_out, coff_SWAP_aouthdr_out,
  coff_SWAP_scnhdr_out,
  FILHSZ_V0, AOUTSZ, SCNHSZ_V01, SYMESZ, AUXESZ, RELSZ_V0, LINESZ,
  FILNMLEN, SYMNMLEN,
#ifdef COFF_LONG_FILENAMES
  TRUE,
#else
...
static const bfd_coff_backend_data ticoff1_swap_table =
{
  ...
  coff_SWAP_filehdr_out, coff_SWAP_aouthdr_out,
  coff_SWAP_scnhdr_out,
  FILHSZ, AOUTSZ, SCNHSZ_V01, SYMESZ, AUXESZ, RELSZ, LINESZ,
  FILNMLEN, SYMNMLEN,
#ifdef COFF_LONG_FILENAMES
  TRUE,
#else
