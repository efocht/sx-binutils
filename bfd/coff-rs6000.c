     ...
     struct internal_ldsym *dst;
{
  ...
  if (bfd_get_32 (abfd, src->_l._l_l._l_zeroes) != 0) {
    memcpy (dst->_l._l_name, src->_l._l_name, bfd_coff_symnmlen(abfd));
  } else {
	 ...
	 const char *name;
{
  ...
  if (len <= bfd_coff_symnmlen(abfd))
    strncpy (ldsym->_l._l_name, name, bfd_coff_symnmlen(abfd));
  else
    {
...
_bfd_xcoff_put_symbol_name (bfd *abfd, struct bfd_strtab_hash *strtab,
			    ...
			    const char *name)
{
  if (strlen (name) <= bfd_coff_symnmlen(abfd))
    {
      strncpy (sym->_n._n_name, name, bfd_coff_symnmlen(abfd));
    }
  else
    {
      bfd_boolean hash;
