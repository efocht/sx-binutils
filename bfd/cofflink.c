...
_bfd_coff_write_global_sym (struct coff_link_hash_entry *h, void *data)
{
  ...
  if (strlen (h->root.root.string) <= bfd_coff_symnmlen(output_bfd))
    strncpy (isym._n._n_name, h->root.root.string,
	     bfd_coff_symnmlen(output_bfd));
  else
    {
      bfd_boolean hash;
      bfd_size_type indx;
