...
#define GET_FSIZE(abfd, ext) \
  H_GET_32 (abfd, ext->x_sym.x_misc.x_fsize)
#endif
#ifndef PUT_FSIZE
#define PUT_FSIZE(abfd, in, ext) \
  H_PUT_32 (abfd, in, ext->x_sym.x_misc.x_fsize)
#endif

#ifndef GET_ARY_DIM
#define GET_ARY_DIM(abfd, ext, n) \
  H_GET_16(abfd, ext->x_sym.x_fcnary.x_ary.x_dimen[n]);
#endif
#ifndef PUT_ARY_DIM
#define PUT_ARY_DIM(abfd, in, ext, n) \
  H_PUT_16(abfd, in, ext->x_sym.x_fcnary.x_ary.x_dimen[n]);
#endif
...
#endif
#ifndef GET_LINENO_SYMNDX
#define GET_LINENO_SYMNDX(abfd, ext) \
  H_GET_32 (abfd, ext->l_addr.l_symndx);
#endif
#ifndef PUT_LINENO_SYMNDX
#define PUT_LINENO_SYMNDX(abfd, val, ext) \
  H_PUT_32 (abfd, val, ext->l_addr.l_symndx);
#endif
...
#endif
/* further header fields also differ on COFF/SX */
#ifndef GET_FILEHDR_NSCNS
#define GET_FILEHDR_NSCNS  H_GET_16
#endif
#ifndef PUT_FILEHDR_NSCNS
#define PUT_FILEHDR_NSCNS  H_PUT_16
#endif
#ifndef GET_FILEHDR_NSYMS
#define GET_FILEHDR_NSYMS  H_GET_32
#endif
#ifndef PUT_FILEHDR_NSYMS
#define PUT_FILEHDR_NSYMS  H_PUT_32
#endif
#ifndef GET_FILEHDR_OPTHDR
#define GET_FILEHDR_OPTHDR  H_GET_16
#endif
#ifndef PUT_FILEHDR_OPTHDR
#define PUT_FILEHDR_OPTHDR  H_PUT_16
#endif
...
#ifndef GET_SYM_OFFSET
#define GET_SYM_OFFSET H_GET_32
#endif
#ifndef GET_SYM_VALUE
#define GET_SYM_VALUE  H_GET_32
#endif
#ifndef GET_SYM_SCNUM
#define GET_SYM_SCNUM  H_GET_16
#endif

#ifndef PUT_SYM_OFFSET
#define PUT_SYM_OFFSET H_PUT_32
#endif
#ifndef PUT_SYM_VALUE
#define PUT_SYM_VALUE  H_PUT_32
#endif
#ifndef PUT_SYM_SCNUM
#define PUT_SYM_SCNUM  H_PUT_16
#endif
...
coff_swap_filehdr_in (bfd * abfd, void * src, void * dst)
{
  ...
  filehdr_dst->f_nscns  = GET_FILEHDR_NSCNS (abfd, filehdr_src->f_nscns);
  filehdr_dst->f_timdat = H_GET_32 (abfd, filehdr_src->f_timdat);
  filehdr_dst->f_symptr = GET_FILEHDR_SYMPTR (abfd, filehdr_src->f_symptr);
  filehdr_dst->f_nsyms  = GET_FILEHDR_NSYMS (abfd, filehdr_src->f_nsyms);
  filehdr_dst->f_opthdr = GET_FILEHDR_OPTHDR (abfd, filehdr_src->f_opthdr);
  filehdr_dst->f_flags  = H_GET_16 (abfd, filehdr_src->f_flags);
#ifdef TIC80_TARGET_ID
  filehdr_dst->f_target_id = H_GET_16 (abfd, filehdr_src->f_target_id);
#endif
#ifdef NEC_SX_COFF
  filehdr_dst->f_flags2 = filehdr_src->f_flags2;
  filehdr_dst->f_flags3 = filehdr_src->f_flags3;
  filehdr_dst->f_flttype = filehdr_src->f_flttype;
#endif /* NEC_SX_COFF */
...
coff_swap_filehdr_out (bfd *abfd, void * in, void * out)
{
  ...
  PUT_FILEHDR_NSCNS (abfd, filehdr_in->f_nscns, filehdr_out->f_nscns);
  H_PUT_32 (abfd, filehdr_in->f_timdat, filehdr_out->f_timdat);
  PUT_FILEHDR_SYMPTR (abfd, filehdr_in->f_symptr, filehdr_out->f_symptr);
  PUT_FILEHDR_NSYMS (abfd, filehdr_in->f_nsyms, filehdr_out->f_nsyms);
  PUT_FILEHDR_OPTHDR (abfd, filehdr_in->f_opthdr, filehdr_out->f_opthdr);
...
#ifdef NEC_SX_COFF
  H_PUT_32 (abfd, 0, filehdr_out->unused);
  H_PUT_8 (abfd, 0, &filehdr_out->pad);
  filehdr_out->f_flags2 = filehdr_in->f_flags2;
  filehdr_out->f_flags3 = filehdr_in->f_flags3;
  filehdr_out->f_flttype = filehdr_in->f_flttype;
#endif /* NEC_SX_COFF */
  ...
  if (ext->e.e_name[0] == 0)
    {
      ...
      in->_n._n_n._n_offset = GET_SYM_OFFSET (abfd, ext->e.e.e_offset);
    }
  else
    {
#if MAX_SYMNMLEN < E_SYMNMLEN
#error E_SYMNMLEN must be <= MAX_SYMNMLEN
#else
#endif
      memcpy (in->_n._n_name, ext->e.e_name, bfd_coff_symnmlen(abfd));
    }

  in->n_value = GET_SYM_VALUE (abfd, ext->e_value);
  in->n_scnum = GET_SYM_SCNUM (abfd, ext->e_scnum);
  ...
  if (in->_n._n_name[0] == 0)
    {
      ...
      PUT_SYM_OFFSET (abfd, in->_n._n_n._n_offset, ext->e.e.e_offset);
    }
  else
    {
#if MAX_SYMNMLEN < E_SYMNMLEN
...
#else
#endif
      memcpy (ext->e.e_name, in->_n._n_name, bfd_coff_symnmlen(abfd));
    }

  PUT_SYM_OFFSET (abfd, in->n_value, ext->e_value);
  PUT_SYM_SCNUM (abfd, in->n_scnum, ext->e_scnum);
...
#error E_FILNMLEN must be <= MAX_FILNMLEN
  ...
  /* if we got to here, we are a sym aux entry */
...
#error we need to cope with truncating or extending DIMNUM
      ...
      in->x_sym.x_fcnary.x_ary.x_dimen[0] =
        GET_ARY_DIM (abfd, ext, 0);
      in->x_sym.x_fcnary.x_ary.x_dimen[1] =
        GET_ARY_DIM (abfd, ext, 1);
      in->x_sym.x_fcnary.x_ary.x_dimen[2] =
        GET_ARY_DIM (abfd, ext, 2);
      in->x_sym.x_fcnary.x_ary.x_dimen[3] =
        GET_ARY_DIM (abfd, ext, 3);
    }

  if (ISFCN (type))
    in->x_sym.x_misc.x_fsize = GET_FSIZE (abfd, ext);
  else
    {
...
#error we need to cope with truncating or extending DIMNUM
#endif
      PUT_ARY_DIM (abfd, in->x_sym.x_fcnary.x_ary.x_dimen[0],
                   ext, 0);
      PUT_ARY_DIM (abfd, in->x_sym.x_fcnary.x_ary.x_dimen[1],
                   ext, 1);
      PUT_ARY_DIM (abfd, in->x_sym.x_fcnary.x_ary.x_dimen[2],
                   ext, 2);
      PUT_ARY_DIM (abfd, in->x_sym.x_fcnary.x_ary.x_dimen[3],
                   ext, 3);
    }

  if (ISFCN (type))
    PUT_FSIZE (abfd, in->x_sym.x_misc.x_fsize, ext);
  else
    {
...
coff_swap_lineno_in (bfd * abfd, void * ext1, void * in1)
{
  ...
  in->l_addr.l_symndx = GET_LINENO_SYMNDX (abfd, ext);
...
coff_swap_lineno_out (bfd * abfd, void * inp, void * outp)
{
  ...
  PUT_LINENO_SYMNDX (abfd, in->l_addr.l_symndx, ext);
...
#ifdef NEC_SX_COFF
  /* stack size */
  aouthdr_int->ssize = H_GET_64(abfd, aouthdr_ext->ssize);
#endif /* NEC_SX_COFF */
...
#ifdef NEC_SX_COFF
  /* nullify dummy and unused fields */
  H_PUT_32(abfd, 0, aouthdr_out->unused);
  H_PUT_32(abfd, 0, aouthdr_out->dummy5);
  H_PUT_32(abfd, 0, aouthdr_out->unused2);
  H_PUT_64(abfd, 0, aouthdr_out->dummy6);
  H_PUT_64(abfd, 0, aouthdr_out->dummy2);
  H_PUT_64(abfd, 0, aouthdr_out->dummy3);
  H_PUT_64(abfd, 0, aouthdr_out->dummy4);

  /* stack size */
  H_PUT_64(abfd, aouthdr_in->ssize, aouthdr_out->ssize);
#endif /* NEC_SX_COFF */
...
#ifdef NEC_SX_COFF
  scnhdr_ext->s_dummy = 0;
  scnhdr_ext->reserved[0] = 0;
  scnhdr_ext->reserved[1] = 0;
  scnhdr_ext->reserved[2] = 0;
#endif /* NEC_SX_COFF */
