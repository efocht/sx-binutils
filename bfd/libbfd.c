...
/*
INTERNAL_FUNCTION
  bfd_write_bigendian_8byte_long_long

SYNOPSIS
  bfd_boolean bfd_write_bigendian_8byte_long_long (bfd *, unsigned long long);

DESCRIPTION
  Write a 8 byte integer @var{i} to the output BFD @var{abfd}, in big
  endian order regardless of what else is going on.  This is useful in
  archives.

*/
bfd_boolean
bfd_write_bigendian_8byte_long_long (bfd *abfd, unsigned long long i)
{
  bfd_byte buffer[8];
  bfd_putb64 ((bfd_vma) i, buffer);
  return bfd_bwrite (buffer, (bfd_size_type) 8, abfd) == 8;
}
