	...
	case bfd_reloc_overflow:
	  {
	    ...
	    if (symndx == -1)
	      ...
	      {
 		strncpy (buf, sym->_n._n_name, bfd_coff_symnmlen(input_bfd));
		buf[bfd_coff_symnmlen(input_bfd)] = '\0';
		name = buf;
	      }
...
static const bfd_coff_backend_data bfd_coff_small_swap_table =
{
  ...
  FILHSZ, AOUTSZ, SCNHSZ, SYMESZ, AUXESZ, RELSZ, LINESZ, FILNMLEN, SYMNMLEN,
