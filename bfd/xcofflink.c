...
_bfd_xcoff_canonicalize_dynamic_symtab (bfd *abfd, asymbol **psyms)
{
  ...
  for (; elsym < elsymend; elsym += bfd_xcoff_ldsymsz(abfd), symbuf++, psyms++)
      ...
      else
	{
	  ...
	  c = bfd_alloc (abfd, (bfd_size_type) bfd_coff_symnmlen(abfd) + 1);
	  if (c == NULL)
	    return -1;
	  memcpy (c, ldsym._l._l_name, bfd_coff_symnmlen(abfd));
	  c[bfd_coff_symnmlen(abfd)] = '\0';
...
xcoff_link_add_dynamic_symbols (bfd *abfd, struct bfd_link_info *info)
{
  ...
  for (; elsym < elsymend; elsym += bfd_xcoff_ldsymsz(abfd))
	...
	{
	  memcpy (nambuf, ldsym._l._l_name, bfd_coff_symnmlen(abfd));
	  nambuf[bfd_coff_symnmlen(abfd)] = '\0';
	  name = nambuf;
	}
